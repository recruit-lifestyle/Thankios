...
BUILT_FRAMEWORK=$(BUILT_BUNDLE)/Contents/Frameworks/$(OUTPUT_FRAMEWORK)
BUILT_EXECUTABLE=$(BUILT_BUNDLE)/Contents/MacOS/$(EXECUTABLE_NAME)
...
installables: clean
	...
	mv -f "$(BUILT_FRAMEWORK)" "$(TEMPORARY_FOLDER)$(FRAMEWORKS_FOLDER)/$(OUTPUT_FRAMEWORK)"
	mv -f "$(BUILT_EXECUTABLE)" "$(TEMPORARY_FOLDER)$(BINARIES_FOLDER)/$(EXECUTABLE_NAME)"
	rm -rf "$(BUILT_BUNDLE)"
